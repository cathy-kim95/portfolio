...
### :exclamation: 소개
> Cash Companion는 예산 추적기와 유사하며, 사용자의 지출을 관리할 수 있는 웹어플리케이션을 개발하였습니다. 사용자는 지출 내역을 입력하면 앱은 사용자 재정에 대한 월별 통계 자료를 계산합니다. 또한 이 앱은 예산 설정 및 분류별 정렬 기능도 제공합니다.
...
- 사용자는 이름, 날짜, 카테고리, 금액 및 설명을 입력하여 새 지출을 추가하는 기능

#### 지출 목록 페이지 
![cc_myspending.png](images/cc_myspending.png)
- 페이지는 활성 계정과 관련된 모든 지출을 사용자가 볼 수 있게 구현
- 지출 추가, 편집 또는 삭제할 수 있는 기능
- 각 지출 카테고리별로 총 지출액이 자동으로 계산되어 표시 및 해당 금액에 해당하는 파이 차트 표시
...
- *팀원과의 협력*: 본래 이 프로젝트는 3명의 다른 팀원과 함께 하는 프로젝트였으나 아무도 프로젝트에 기여하지 않아 혼자 하게 되는 상황에 놓였습니다.
- *chart.js 사용법 익히기*: 차트를 구현해보는 것은 처음이라 파이 차트를 구현할 때 어려움을 겪었습니다.
- *다큐멘트 및 WIKI 작성*: 처음으로 개발하는 툴에 대해 다큐멘트와 WIKI를 작성해야 해서 어떻게 써야할지 막막했습니다.

##### 극복 과정

- *차라리 혼자*: 항상 어플리케이션이나 웹사이트를 만들때 팀원이 있었지만 이번엔 아예 혼자 해볼까? 하는 생각이 들었습니다. 제 페이스대로 천천히 공부해가며 사이트를 구축하기 시작했습니다.
- *온라인 자료 활용*: chart.js의 공식 문서나 온라인 샘플, 커뮤니티 포럼 등을 활용하여 문제 해결에 필요한 정보를 얻었습니다.
- *끈기 있는 노력*: 꾸준히 노력하고 시행착오를 반복하면서 깔끔하고 자세한 다큐멘트 및 WIKI 를 작성을 할 수 있었습니다.

##### 결과
- *독립적 업무 수행 능력 향상*: 프로젝트를 진행하면서 독립적으로 다큐멘트 및 WIKI를 작성하고 차트를 구현하는 등의 업무를 수행하면서 독립적으로 업무를 처리하는 능력이 향상되었습니다. 혼자서 프로젝트의 모든 과정을 관리하고 진행함으로써 프로젝트에 대한 전반적인 이해도가 높아지고, 독립적으로 업무를 수행할 수 있는 능력이 강화되었습니다.
