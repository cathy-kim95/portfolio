...
#### :pushpin: 개인기여도
- `useState` 과 `useEffect` 같은 React 훅을 사용하여 요소를 처리하고, 팝업을 만들어 수정/저장할 수 있게 구현
- `handleElements`로 맵을 사용하여 현재 웹사이트에 표시된 요소를 내보낼 수 있도록 구현
- JSON으로 변환된 XML 파일을 순회해 노드를 연결하고 웹사이트에 그래프를 시각화
...
#### :pushpin: 개인기여도
- `MongoDB` 를 통해 입법 청문회에 대한 정보를 연도, 법안 번호, 법안 유형, 날짜/시간, 등등으로 컬렉션으로 저장
...
### :exclamation: 소개
> Cash Companion는 예산 추적기와 유사한, 사용자의 지출을 관리할 수 있는 웹어플리케이션입니다. 사용자가 지출 내역을 입력하면 이 웹은 사용자 재정에 대한 월별 통계 자료를 계산하고 분류별 정렬 기능도 제공합니다.
...
- `React-Bootstrap` 으로 사용자가 편하게 쓸 수 있는 UI를 구현
